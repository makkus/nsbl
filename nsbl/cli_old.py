...
def describe_tasks(ctx, config, format, pager):
    ...
    chain = [
        ...
        frkl.FrklProcessor(task_format),
...
def extract_inventory(ctx, config, target, static, relative):
    ...
    if static:
        ...
        nsbl.inventory.write_inventory_file_or_script(
            "/tmp/inventory", extract_vars=True
        )
    else:
        os.makedirs("/tmp/inventory")
        nsbl.inventory.write_inventory_file_or_script(
            "/tmp/inventory", extract_vars=False, relative_paths=relative
        )
